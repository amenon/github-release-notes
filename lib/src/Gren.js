...
class Gren {
    constructor(props = {}) {
        ...
        this.options.deleteReleaseWithTags = utils.convertStringToArray(deleteReleaseWithTags);
    ...
    /**
     * Delete a release from a given tag (in the options)
     *
     * @since 0.18.0
     * @private
     *
     ...
     * @return {Promise}
     */
    async _deleteRelease(releaseId) {
        const loaded = utils.task(this, 'Deleting release');
        const { data: release } = await this.repo.deleteRelease(releaseId);

        loaded(chalk.green(`${release.name} has been successfully deleted!`));
    ...
    /**
     * Creates the options to make/delete the release
     *
     * @since 0.2.0
     * @private
     *
    ...
    _prepareRelease(block) {
        ...
        if (this.options.deleteReleaseWithTags.includes(block.release)) {
            if (!block.id) {
                console.warn(chalk.black(chalk.bgYellow('No release to delete')));
                return Promise.resolve();
            }
            return this._deleteRelease(block.id);
        }
